...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      setChatError(e.response?.data?.detail || 'Failed to get a response.')
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setAnalysisError(e.response?.data?.detail || 'Analysis failed.')
  ...
  const runSuggestions = async () => {
    ...
    try {
      ...
      setSuggestionsError(e.response?.data?.detail || 'Failed to get suggestions.')
  ...
  const runCaptions = async (style: CaptionStyle = captionStyle) => {
    ...
    try {
      ...
      setCaptionsError(e.response?.data?.detail || 'Failed to generate captions.')
      ...
      return (
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 scrollbar-none">
            {messages.length === 0 ? (
              ...
              <>
                ...
                {chatError && <p className="text-[11px] text-danger bg-danger/10 border border-danger/20 rounded-xl px-3 py-2">{chatError}</p>}
              </>
            )}
            <div ref={bottomRef} />
          </div>
          ...
          <div className="border-t border-border bg-surface-raised p-2.5 flex gap-2 items-end shrink-0">
            {messages.length > 0 && (
              <button onClick={() => { setMessages([]); setChatError(null) }} title="Clear"
      ...
      return (
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2 scrollbar-none">
            {analysisError && <p className="text-[11px] text-danger bg-danger/10 border border-danger/20 rounded-xl px-3 py-2">{analysisError}</p>}
    ...
    if (mode === 'suggestions') {
      return (
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2 scrollbar-none">
            {suggestionsError && <p className="text-[11px] text-danger bg-danger/10 border border-danger/20 rounded-xl px-3 py-2">{suggestionsError}</p>}
      ...
      return (
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="p-3 space-y-2 shrink-0">
            ...
            <button onClick={() => runCaptions()} disabled={!activeFile || captionsLoading}
              className="w-full py-2 rounded-xl bg-teal hover:bg-teal/90 text-xs font-bold text-white transition-all active:scale-95 disabled:opacity-30 flex items-center justify-center gap-2 shadow-md">
              {captionsLoading
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2 scrollbar-none">
            {captionsError && <p className="text-[11px] text-danger bg-danger/10 border border-danger/20 rounded-xl px-3 py-2">{captionsError}</p>}
